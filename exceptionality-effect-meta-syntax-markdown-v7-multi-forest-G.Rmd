---
title: "Past behavior normality and regret/counterfactuals meta-analysis"
description: null
output:
  pdf_document:
    fig_height: 6
...
X<-"Past behavior normality"
Y<-"regret/counterfactuals"

#what is the name of the datafile ?
filename<-"past-behavior-meta-coding.csv"
...
#Set system language to English (added by Lucas 8/9/2017, because my system R ran in German)
Sys.setenv(LANG = "en")

# Installing all the required packages for this analysis (if not already installed)
list.of.packages <- c("plyr", "dplyr", "rstudioapi", "foreign", "metafor","pequod", "csv", "ggplot2", "xlsx",  "psych", 
...
#Exception/routine main effect analyses#
...
This analysis is based on `r allexpcollapsed$k` experimental studies that evaluated the impact of `r X` over `r Y`. 
...
A random effects meta-analysis was conducted (k=`r allexpcollapsed$k`) to explore the effect of `r X` on `r Y`. The average effect-szie was Hedge's g = `r round(allexpcollapsed$b,3)`, (p = `r ifelse(allexpcollapsed$pval < 0.001, "< .001", round(allexpcollapsed$pval,3))`, 95% CI [`r round(allexpcollapsed$ci.lb, 2)`, `r round(allexpcollapsed$ci.ub, 2)`]). See table below.
...
A random effects meta-analysis was conducted (k=`r allcompcollapsed$k`) to explore the effect of `r X` on `r Y`. The average effect-szie was Hedge's g = `r round(allcompcollapsed$b,3)`, (p = `r ifelse(allcompcollapsed$pval < 0.001, "< .001", round(allcompcollapsed$pval,3))`, 95% CI [`r round(allcompcollapsed$ci.lb, 2)`, `r round(allcompcollapsed$ci.ub, 2)`]). See table below.

`r kable(allcompcollapsed.restable, digits=3, row.names=NA, col.names=c("Hedge's g","SE", "p", "CI Lower", "CI Upper", "k"), caption="Meta-Analysis Summary Table")`

###Does the effect-size vary across studies?###

A Cochran's Q test was conducted to examine whether variations in the observed effect-size are likely to be attributable soley to sampling error (Q~(df=`r allcompcollapsed$df.Q`)~=`r round(allcompcollapsed$QE,2)`, p=`r ifelse(allcompcollapsed$QEp < 0.001, "< .001", round(allcompcollapsed$QEp,3))`). `r ifelse(allcompcollapsed$QEp < 0.05, "The variation in effect-size is greater than would be expected from sampling error alone. It appears that the true effect varies betweeen studies.", "There is no evidence that the true effect size varies between studies.")`

The I^2^ statistics indicates the *proportion* of variance in the observed effect attributable to sampling error. In this instance, the I^2^ = `r round(allcompcollapsed$I2,2)`%. 

Heterogeneity statistics are summarised below
`r kable(allcompcollapsed.hettable, digits=3, row.names=NA, col.names=c("Q", "p", "I^2^"),caption="Heterogeneity Summary Table")`
